...
def test_authentication_retry_limit_compliance(fail_count: int) -> None:
    ...
    **Validates: Requirements 1.4**
    ...
    async def run_test():
        with patch.object(client, '_refresh_access_token', side_effect=mock_refresh):
            with patch('asyncio.sleep', new_callable=AsyncMock):
                try:
                    await client.authenticate()
                except NestAuthenticationError:
                    pass  # Expected if fail_count >= MAX_CONNECTION_RETRIES
...
def test_get_thermostat_retry_limit_compliance(fail_count: int) -> None:
    ...
    **Validates: Requirements 1.4**
    ...
    async def run_test():
        with patch.object(client, '_fetch_thermostat_data', side_effect=mock_fetch):
            with patch('asyncio.sleep', new_callable=AsyncMock):
                try:
...
def test_set_temperature_retry_limit_compliance(fail_count: int) -> None:
    ...
    **Validates: Requirements 2.3**
    ...
    async def mock_set_temp(target: float):
        if counter.should_fail():
            raise NestAPIError("Simulated API failure")
    ...
    async def run_test():
        with patch.object(client, '_set_temperature_api', side_effect=mock_set_temp):
            with patch.object(client, 'get_thermostat_data', side_effect=mock_get_data):
                with patch('asyncio.sleep', new_callable=AsyncMock):
...
def test_authentication_fails_after_max_retries() -> None:
    ...
    **Validates: Requirements 1.4**
    ...
    async def run_test():
        with patch.object(client, '_refresh_access_token', side_effect=mock_refresh):
            with patch('asyncio.sleep', new_callable=AsyncMock):
                with pytest.raises(NestAuthenticationError):
                    await client.authenticate()
